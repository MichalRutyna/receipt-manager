...
class Window(tkinter.Toplevel):
    def __init__(self, master):
        ...
        self.title("Budget manager")
        self.iconbitmap(default="icon.ico")
        self.ikona = ImageTk.PhotoImage(Image.open('icon.png').resize((17, 17)))
        self.update()
...
def brighten(color):
    ...
    # rgb -> hex + whitening
    return rgb2hex(min(int(1.2 * (R + 100)), 255),
                   min(int(1.2 * (G + 100)), 255),
                   min(int(1.2 * (B + 100)), 255))
